...
export const getPossibleMove = async (io, socket, payload) => {
    validateAuthToken(payload.token, socket.id)
        ...
        if (!tokenValidate.validate)
            errors.token.push(tokenValidate.message);
        ...
        else if (!validatePosition(payload.position))
            errors.position.push('Invalid position.');
...
export const moveTile = async (io, socket, payload) => {
    validateAuthToken(payload.token, socket.id)
        .then(async (tokenValidate) => {
        /**
         * validate token and gameId
        ...
        else if (!validatePosition(payload.from))
            errors.from.push('Invalid position.');
        if (!payload.to || payload.to === '')
            errors.to.push('to is required.');
        else if (!validatePosition(payload.to))
            errors.to.push('Invalid position.');
        if (errors.token.length > 0 ||
            errors.gameId.length > 0 ||
